			...
			<form class="layui-form">
				...
				<table class="list-table">
					<thead>
						...
						<th style="text-align: center;">红包抵扣价格</th>
					...
					<tbody class="table-list-box">
						<c:forEach items="${resultData.dataList}" var="v" varStatus="i">
							<tr>
								...
								<c:choose>
								<c:when test="${v.zfStyle==1}">支付宝</c:when>
								<c:when test="${v.zfStyle==2}">微信</c:when>
								...
								<c:when test="${v.status==1 || v.beforeTheAuditStatus==1}">已付款</c:when>
								<c:when test="${v.status==2 || v.beforeTheAuditStatus==2}">卖家发货</c:when>
								...
								<c:if test="${v.status==1 || v.beforeTheAuditStatus==1}">
								...
								<c:if test="${v.status==2 || v.status==4 || v.beforeTheAuditStatus==2}">
									
								<td style="text-align: center;">
								<a href="javascript:;" class="layui-btn layui-btn-normal layui-btn-small" title="修改物流" onclick="deliveryGoods(${v.id})">修改物流</a>
									</br></br>
									<a href="javascript:;" class="layui-btn layui-btn-warm layui-btn-small" title="查看物流" onclick="lookCourier(${v.id})">查看物流</a></td>
								</c:if>
