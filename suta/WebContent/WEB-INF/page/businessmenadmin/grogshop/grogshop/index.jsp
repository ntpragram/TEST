		...
		<div class="layui-tab-item layui-show">
			...
			<form class="layui-form layui-form-pane search-form">
				<!--div class="layui-form-item">
					...
					<div class="layui-input-inline" style="width: 80px;margin-right:10px">
						<select id="sh" style="display:block;height:38px;width: 80px;" onchange="pageJump(1)">
							<option value="-1" <c:if test="${resultData.sh == -1}">selected</c:if>>全部</option>
							<option value="0" <c:if test="${resultData.sh == 0}">selected</c:if>>待审核</option>
							<option value="1" <c:if test="${resultData.sh == 1}">selected</c:if>>通过</option>
							<option value="2" <c:if test="${resultData.sh == 2}">selected</c:if>>不通过</option>
						</select>
					</div>

					<label class="layui-form-label" style="width: 60px;">星级</label>
					<div class="layui-input-inline" style="width: 80px;margin-right:10px">
						<select id="starLevel" style="display:block;height:38px;width: 80px;" onchange="pageJump(1)">
							<option value="-1" <c:if test="${resultData.starLevel == -1}">selected</c:if>>全部</option>					
							<option value="1" <c:if test="${resultData.starLevel == 1}">selected</c:if>>一星</option>
							<option value="2" <c:if test="${resultData.starLevel == 2}">selected</c:if>>二星</option>
							<option value="3" <c:if test="${resultData.starLevel == 3}">selected</c:if>>三星</option>
							<option value="4" <c:if test="${resultData.starLevel == 4}">selected</c:if>>四星</option>
							<option value="5" <c:if test="${resultData.starLevel == 5}">selected</c:if>>五星</option>
							<option value="0" <c:if test="${resultData.starLevel == 0}">selected</c:if>>其他</option>
						</select>
					</div>

					<label class="layui-form-label" style="width: 60px;">热门酒店</label>
					<div class="layui-input-inline" style="width: 80px;margin-right:10px">
						<select id="hot" style="display:block;height:38px;width: 80px;" onchange="pageJump(1)">
							<option value="-1" <c:if test="${resultData.hot == -1}">selected</c:if>>全部</option>
							<option value="0" <c:if test="${resultData.hot == 0}">selected</c:if>>否</option>
							<option value="1" <c:if test="${resultData.hot == 1}">selected</c:if>>是</option>
						</select>
					</div>

					<label class="layui-form-label" style="width: 60px;">精品酒店</label>
					<div class="layui-input-inline" style="width: 80px;margin-right:10px">
						<select id="Boutique" style="display:block;height:38px;width: 80px;" onchange="pageJump(1)">
							<option value="-1" <c:if test="${resultData.Boutique == -1}">selected</c:if>>全部</option>
							<option value="0" <c:if test="${resultData.Boutique == 0}">selected</c:if>>否</option>
							<option value="1" <c:if test="${resultData.Boutique == 1}">selected</c:if>>是</option>

						</select>
					</div>

					<label class="layui-form-label" style="width: 60px;">上下架</label>
					<div class="layui-input-inline" style="width: 80px;margin-right:10px">
						<select id="shelve" style="display:block;height:38px;width: 80px;" onchange="pageJump(1)">
							<option value="-1" <c:if test="${resultData.shelve == -1}">selected</c:if>>全部</option>
							<option value="0" <c:if test="${resultData.shelve == 0}">selected</c:if>>上架</option>
							<option value="1" <c:if test="${resultData.shelve == 1}">selected</c:if>>下架</option>
						</select>
					</div>
			...
			<form class="layui-form layui-form-pane search-form">
				<div class="layui-form-item" style="margin-top: 5px;">
					<div class="layui-btn-group">
						...
						<a style="color: #fff;position: relative;top: 1px;">添加酒店</a>
						</c:when>
						</c:choose>
						</span>
					</div>
			...
			<form class="layui-form">
				...
				<table class="list-table">
					<thead>
						...
						<th style="text-align: center;">热门酒店</th>
						<!--<th style="text-align: center;">轮播图</th>-->
						<th style="text-align: center;">厅管理</th>
						<th style="text-align: center;">套餐管理</th>
						<th style="text-align: center;">菜单管理</th>
					...
					<tbody class="table-list-box">
						<c:forEach items="${resultData.dataList}" var="v" varStatus="i">
							<tr>
								...
								<td style="text-align: center;"><a href="<%=basePath%>businessmenadmin/room/getRoomList?gsid=${v.gsid}"  class="layui-btn layui-btn-small" title="厅管理">厅管理</a></td>
								<td style="text-align: center;"><a href="<%=basePath%>businessmenadmin/menu/getTaoCanList?gsid=${v.gsid}&type=${resultData.type}"  class="layui-btn layui-btn-small" title="套餐管理">套餐管理</a></td>
								<td style="text-align: center;"><a href="<%=basePath%>businessmenadmin/menu/getMenuList?gsid=${v.gsid}"  class="layui-btn layui-btn-small" title="菜单管理">菜单管理</a></td>
