							...
							<div class="layui-form-item" style="margin-top: 30px">
									...
									<select name="eid" id="eid">
										...
										<c:forEach items="${list}" var="v">		
											<option value="${v.id}" <c:if test="${v.id==eid}">selected</c:if>>${v.title}</option>
										</c:forEach>	
									</select>
									</div>
							</div>
							...
							<div class="layui-form-item" style="margin-top: 30px">
									...
									<div class="layui-input-block">
										<input type="text" name="answer" id="answer" 
											value="${answers}" placeholder="请输入内容"  lay-verify="" autocomplete="off"
											class="layui-input">
									</div>
								</div>
						...
						<script type="text/javascript">
							...
							layui.use(['element','form','upload','layedit'], function() {
								...
								form.on('submit(form)', function(data) {
									var eid = $("#eid").val();
									if(eid==""){
										layer.msg("请选择测评类别", {icon: 2,anim: 6, time: 1000});
										return;
									}									
									...
									$.post(sysUrl+'evaluate/modifyResultSubmit',json,function(data){
