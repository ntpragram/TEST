...
<div class="layui-tab layui-tab-brief main-tab-container">
	<ul class="layui-tab-title main-tab-title">
		<li class="layui-this" style="border-bottom: 0px;top:0px;">酒店订单</li>
						...
						<c:forEach items="${resultData.dataList}" var="v" varStatus="i">
							<tr>
								...
								<td style="text-align: center;">
									<select style="display:block;margin: 0 auto;" onchange="audit(${v.oid})" id="audit${v.oid}">
										<option value="0" <c:if test="${v.ostatus == '0'}">selected</c:if>>待付款</option>
										<option value="1" <c:if test="${v.ostatus == '1'}">selected</c:if>>已确认</option>
										<option value="2" <c:if test="${v.ostatus == '2'}">selected</c:if>>已取消</option>
									</select>
								</td>
