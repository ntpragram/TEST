		...
		<div class="layui-tab-item layui-show">
			...
			<form class="layui-form">
				...
				<table class="list-table">
					...
					<thead>
						<tr>
						<th class="first"><button class="layui-btn layui-btn-small" lay-submit lay-filter="delete">推送</button></th>
							<th colspan="10">
								<div id="demo8">
...
<script type="text/javascript">
	...
	var rid = "${resultData.rid}";
	var urlWhere = '?offset='+pageNumber+'&limit='+page_size+'&search='+search+'&startTime='+startTime+'&rid='+rid;		
	
	function pageJump(pageNumber){
		search = $("#search").val();
		startTime = $("#startTime").val();
		rid = $("#rid").val();
		window.location.href = '?offset='+pageNumber+'&limit='+page_size+'&search='+search+'&startTime='+startTime+'&rid='+rid;
	}
    ...
    layui.use(['element', 'laypage', 'layer', 'form', 'laydate'], function(){
        ...
        jq('.del_btn').click(function(){
            ...
            layer.confirm('确定推送账号为【'+name+'】的用户吗?', function(index){
                loading = layer.load(2, {
                    shade: [0.2,'#000'] //0.2透明度的白色背景
                });
				var json = {id:id};
...
//判断是否有选项
			...
			//批量删除
            layer.confirm('确定批量推送选中的记录吗?', function(index){
                loading = layer.load(2, {
                    shade: [0.2,'#000'] //0.2透明度的白色背景
                });
                ...
                var param = {id:strlist,rid:rid};
				// console.log("param:"+JSON.stringify(param));
                jq.post('pushOneMember',param,function(data){
					layer.close(loading);
					if(data.code==200){
						layer.msg(data.message, {icon: 1, time: 1000}, function(){
							window.location.href = urlWhere;
