...
<div class="layui-tab layui-tab-brief main-tab-container">
	<ul class="layui-tab-title main-tab-title">
		<li class="layui-this" style="border-bottom: 0px;top:0px;">示例列表</li>
		<div class="main-tab-item" style="left: 10px;top:0px;">示例管理</div>
	</ul>
		...
		<!-- 搜索 -->
          ...
          <form class="layui-form layui-form-pane search-form">
              <div class="layui-form-item">
                  ...
                  <label class="layui-form-label" style="width: 60px;margin-left:10px">所属人群</label>
					<div class="layui-input-inline" style="width: 100px;">
						<select id="insId" name="insId" style="display:block;height:38px;">
							<option value="">全部</option>
							<c:forEach items="${resultData.sampleAttributeList}" var="vs" >
							<option value="${vs.id }" <c:if test="${vs.id==resultData.insId}">selected</c:if>>${vs.title}</option>
							</c:forEach>
						</select>
					</div>
        ...
        <div class="layui-btn-group">
          ...
          <span class="layui-btn layui-btn-small modal-catch" onclick="toAdd()">
			...
			<form class="layui-form">
                ...
                <input type="hidden" id="modelName"  value="sampleAttribute" >
						...
						<c:forEach items="${resultData.dataList}" var="v" varStatus="i">
							<tr>
								...
								<td style="text-align: center;" class="table-img">
									<div class="img"><img src="<%=basePath%>${v.pic}" alt="..." class="img-thumbnail" style="width: 100px;"></div>
								</td>
								<td style="text-align: center;" class="table-img">
									<div class="img"><img src="<%=basePath%>${v.bigPic}" alt="..." class="img-thumbnail" style="width: 100px;"></div>
								</td>
								<td style="text-align: center;" class="table-img">
									<div class="img"><img src="<%=basePath%>${v.smallPic}" alt="..." class="img-thumbnail" style="width: 100px;"></div>
								</td>
								<td style="text-align: center;">${v.introduce}</td>
								<td style="text-align: center;">${v.insTitle}</td>
								<td style="text-align: center;"><a href="javascript:;" class="layui-btn layui-btn-warm layui-btn-small" title="项目列表" onclick="projectList(${v.id})">项目列表</a></td>
...
<script type="text/javascript">
	...
	var insId = "${resultData.insId}";
	function pageJump(pageNumber){
		search = $("#search").val();
		insId = $("#insId").val();
		window.location.href = 'getSampleList?offset='+pageNumber+'&limit='+page_size+'&search='+search+'&insId='+insId;
	}
                ...
                jq.post('delete',json,function(data){
					...
					if(data.code==200){
						layer.msg(data.message, {icon: 1, time: 1000}, function(){
							window.location.href = '?offset='+pageNumber+'&limit='+page_size+'&search='+search+'&insId='+insId;
						});
					}else{
						layer.msg(data.message, {icon: 2,anim: 6, time: 1000});
					}
...
//判断是否有选项
    ...
    layer.open({
      ...
      area: ['90%','90%'],
      title: '添加',
      content: sysUrl+'sample/projectList?sid='+sid
    });
...
function toMod(id){
    ...
    layer.open({
      ...
      area: ['80%','80%'],
      title: '修改',
      content: sysUrl+'sample/toModify?id='+id 
    });
