...
<div class="layui-tab layui-tab-brief main-tab-container">
	<ul class="layui-tab-title main-tab-title">
		<li class="layui-this" style="border-bottom: 0px;top:0px;"><c:if test="${resultData.tag==0}">云商城列表管理</c:if><c:if test="${resultData.tag==1}">杉生活列表管理</c:if></li>
		<div class="main-tab-item" style="left: 10px;top:0px;">
        	<c:if test="${resultData.tag==0}">云商城列表</c:if><c:if test="${resultData.tag==1}">杉生活列表</c:if>
        </div>
	</ul>
	<div class="layui-tab-content">
        ...
        <!-- 每页数据量 -->
			...
			<form class="layui-form">
                ...
                <input type="hidden" id="modelName"  value="integralProduct" >
				<table class="list-table">
					<thead>
                        ...
                        <th>大图</th>  
                        <th>小图</th>  
                        <th><c:if test="${resultData.tag==0}">所需云币</c:if><c:if test="${resultData.tag==1}">所需杉币</c:if></th>
                        <c:if test="${resultData.tag==1}">
                         <th>所需金额</th>
                         </c:if>
                        <th>库存</th>
                        <th>销量</th>
						...
						<c:forEach items="${resultData.dataList}" var="v" varStatus="i">
							<tr>
                                ...
                                <td style="text-align: center;" class="table-img">
                                  ...
                                  <td style="text-align: center;" class="table-img">
                                    <div class="img"><img src="<%=basePath%>${v.smallPic}" alt="..." class="img-thumbnail" style="width: 80px;height: 80px;"></div>
                                </td>
                                ...
                                <td style="text-align: center;">${v.inventory}</td> 
                                 <td style="text-align: center;">${v.salesVolume}</td> 
								...
								<td class="table2-edit" style="text-align: center;">
									<c:if test="${v.tj==0}">
									<a href="javascript:;" onclick="modifyTuijian(${v.id},1)" class="layui-btn layui-btn-normal" feedback-id="${v.id}" feedback-name="${v.title}" title="推荐" >
									<i class="layui-icon">推荐</i>
									</a>
									</c:if>

									<c:if test="${v.tj==1}">
									<a href="javascript:;" onclick="modifyTuijian(${v.id},0)" class="layui-btn layui-btn-warm" feedback-id="${v.id}" feedback-name="${v.title}" title="取消推荐" >
									<i class="layui-icon">取消推荐</i>
									</a>
									</c:if>
								</td>
								
								<td style="text-align: center;">
                                <a href="javascript:;" class="layui-btn layui-btn-warm layui-btn-small" title="轮播图管理" onclick="picList(${v.id})">轮播图列表</a></td>
...
<script type="text/javascript">
    ...
    var urlWhere ='?offset='+pageNumber+'&limit='+page_size+'&search='+search+'&tag='+tag;
	function pageJump(pageNumber){
		...
		window.location.href = '?offset='+pageNumber+'&limit='+page_size+'&search='+search+'&tag='+tag;
	}
    ...
    layui.use(['element', 'laypage', 'layer', 'form'], function(){
		...
		modifyTuijian = function(id,tj){
			...
			var json = {id:id,tj:tj};
			jq.post('modifyTuiJian',json,function(data){
...
function toAdd(){
    ...
    layer.open({
      ...
      area: ['90%','90%'],
      title: '添加',
      content: sysUrl+'integralProduct/toAdd?tag='+tag
    });
...
function toMod(id){
    ...
    layer.open({
      ...
      area: ['90%','90%'],
      title: '修改',
      content: sysUrl+'integralProduct/toModify?id='+id 
    });
}
...
function picList(id){
    ...
    layer.open({
      ...
      area: ['90%','90%'],
      title: '轮播图列表',
      content: sysUrl+'integralProduct/getPicList?pid='+id 
    });
}
